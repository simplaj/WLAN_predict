...
S2s = pd.read_csv("score__.csv")
C_s = pd.read_csv("scores/score_new.csv")
C_s = C_s.sort_values(by='SP', ascending=True)
c_s = C_s['Score']
s2s = S2s.values[1:, 4]
C_s = C_s.values[1:s2s.shape[0]+1, :]
vae = Vae.values[1:s2s.shape[0]+1, :]
dae = Dae.values[1:s2s.shape[0]+1, :]
mea = Mea.values[1:s2s.shape[0]+1, :]
c_s = c_s.values[1:s2s.shape[0]+1]
output = np.append(mea, c_s.reshape(s2s.shape[0], 1), axis=-1)
...
save.to_csv('score_5.csv', index=False, header=True)
